...
*   **富文本/多媒体交互**：
    ...
    *   **@音乐 功能**：发送 `@音乐` 可随机获取一首热门歌曲，以精美卡片形式展示并支持在线播放。
    *   **@天气 功能**：发送 `@天气 [城市]`（如 `@天气 北京`）可获取该城市的实时天气及预报卡片。
    *   **@新闻 功能**：发送 `@新闻` 可获取今日热点新闻摘要，支持折叠展开查看详情。
...
*   **普通消息**：直接广播到房间。
*   **指令消息**：
    *   `@电影 [URL]`：解析视频链接并转换为 iframe。
    *   `@音乐`：调用第三方 API 获取随机音乐并渲染为音乐卡片。
    *   `@天气 [城市]`：调用天气 API 获取数据并渲染为动态天气卡片。
    *   `@新闻`：调用新闻 API 获取热点列表并渲染为折叠式新闻列表。
